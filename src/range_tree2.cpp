...
public:
  ...
  const Point &sample(std::mt19937 *rng_ptr_)
  {
    ...
    distribution.param(std::uniform_int_distribution<std::size_t>::param_type(0, n - 1));
    return v[distribution(*rng_ptr_)];
  }
};
...
struct BoundingBox
{
  ...
  {
    return !right ? BoundingBox{x_min, y_min, x_mid, y_max} : BoundingBox{x_mid, y_min, x_max, y_max};
  }
  BoundingBox split_y(int top) const
  {
    return !top ? BoundingBox{x_min, y_min, x_max, y_mid} : BoundingBox{x_min, y_mid, x_max, y_max};
  }
};
...
{
public:
  RangeTree2(const BoundingBox &boundary, int depth, std::vector<int> seeds = {})
      : depth_(depth)
  {
...
// [[Rcpp::export]]
Eigen::MatrixXd test_range2(const Eigen::MatrixXd &points, const Eigen::MatrixXd &query,
                            int depth = 5)
{
  RangeTree2 rangetree(BoundingBox{0.0, 0.0, 1.0, 1.0}, depth);
